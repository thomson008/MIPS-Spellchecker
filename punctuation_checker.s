...
tokenLoop:
        ...
        beq $t2, 1, checkPunct          # if it is a punctuation, go check if t is correct
        beq $t2, 2, printCorrect        # if it is a spece, simpy print it without any checking
...
checkPunct:
	addi $s4, $s5, -201             # 
	lb $a3, tokens($s4)             # get the first char from previous token
	li $s6, 32
	beq $a3, $s6, printWrong        # if it is a space,print the token with underscores
	blez $a3, printWrong            # if the first char of first token is 0, meaning that he punctuation is the first char in the sentence, also print wrong 
	addi $s4, $s5, 201              #
	lb $a0, tokens($s4)             # get the first char from next token
	jal checkType			# check its type
	beq $t2, $0, printWrong		# if it is alphabetic, print wrong
	addi $s4, $s5, 202              ################################################## more than one space after a punctuation?
	lb $a3, tokens($s4)             #######################
	beq $a3, 32, printWrong         #################################################  
	lb $a3, tokens($s5)             # get the first char from current token
	li $s6, 46
	bne $a3, $s6, isNextZero         # if it is not a dot, go to check if it is not repeated
	j isNextDotOrZero               # else, go to check if the next character is a dot or zero

isNextZero:
	addi $s4, $s5, 1                #
	lb $a3, tokens($s4)             # get the next char from current token
	blez $a3, printCorrect          # if it is a zero, print correctly 
	j printWrong                    # else, print wrong
	
isNextDotOrZero:
	addi $s4, $s5, 1                # 
	lb $a3, tokens($s4)             # get the next char from the current token
	blez $a3, printCorrect          # if it is a zero, print correctly
	li $s6, 46
	bne $a3, $s6, printWrong        # if it is neither a zero nor another dot, print wrong (with underscores)
	addi $s4, $s5, 2                #
	lb $a3, tokens($s4)             # get the 3rd char from current token
	bne $a3, $s6, printWrong        # if it is not a 3rd dot, print wrong
	addi $s4, $s5, 3                # 
	lb $a3, tokens($s4)             # get the 4th char from current token
	blez $a3, printCorrect		# if it terminates the token, print correctly
	j printWrong
